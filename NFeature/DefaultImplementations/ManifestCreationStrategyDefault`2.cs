...
namespace NFeature.DefaultImplementations
{
		...
		where TTenantEnum : struct
	{
        ...
        public IFeatureManifest<TFeatureEnum> CreateFeatureManifest(string XMLconfigFilename) {
            FeatureSetting<TFeatureEnum, TTenantEnum>[] featureSettings = _featureSettingRepository.GetFeatureSettingsFromXML(XMLconfigFilename);

			var manifest = new FeatureManifest<TFeatureEnum>();

			foreach (var setting in featureSettings) {
				var isAvailable = _featureSettingService.AllDependenciesAreSatisfiedForTheFeatureSetting(setting, new EmptyArgs(), XMLconfigFilename);
